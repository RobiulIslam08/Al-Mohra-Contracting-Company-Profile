    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-16">
        ...
        <div className="scroll-reveal-left p-8 rounded-2xl relative overflow-hidden"
          ...
          <h3 className="text-[#0B1A33] text-xl font-bold mb-4" style={{ fontFamily: 'Playfair Display, serif' }}>
            Vision Statement
          </h3>
      ...
      <div className="mb-12">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {VALUES.map((v, i) => (
            <div key={v.en}
              className={`p-6 rounded-2xl text-center scroll-reveal stagger-${i+1} cursor-default`}
              ...
              <p className="text-xs font-black tracking-widest uppercase mb-3 text-[#0B1A33]">{v.en}</p>
              <p className="text-xs leading-relaxed" style={{ color: 'rgba(11,26,51,0.65)' }}>{v.desc}</p>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {BOTTOM_VALUES.map(({ title, desc }, i) => (
            <div key={title} className="text-center py-4"
              ...
              <p className="text-xs" style={{ color: 'rgba(11,26,51,0.6)' }}>{desc}</p>
            </div>
          ))}
        </div>
      </div>
