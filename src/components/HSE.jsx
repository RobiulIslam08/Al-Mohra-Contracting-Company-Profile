...
const HSE = () => (
  <section id="hse" className="w-full py-24 relative overflow-hidden"
    ...
    <div className="orb orb-gold absolute" style={{ width: '450px', height: '450px', top: '-50px', left: '-50px', opacity: 0.04, animationDuration: '18s' }} />
    ...
    <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="text-center mb-14 scroll-reveal">
        ...
        <p className="text-sm max-w-2xl mx-auto leading-relaxed" style={{ color: '#5a6a82' }}>
      ...
      <div className="grid grid-cols-3 gap-4 mb-14 scroll-reveal">
        {[
          ...
          <div key={label} className="text-center p-6 rounded-2xl"
            ...
            <p className="text-[9px] mt-1" style={{ color: 'rgba(11,26,51,0.4)' }}>To be confirmed by client</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-14">
        {HSE_FEATURES.map((f, i) => (
          <div key={f.title}
            className={`p-6 rounded-2xl scroll-reveal stagger-${i+1}`}
            ...
            <h3 className="font-bold text-[#0B1A33] text-sm mb-2">{f.title}</h3>
            <p className="text-xs leading-relaxed" style={{ color: 'rgba(11,26,51,0.65)' }}>{f.desc}</p>
          </div>
        ))}
      </div>
      ...
      <div className="p-8 rounded-2xl scroll-reveal"
        style={{ background: '#FAF7F2', border: '1px solid rgba(11,26,51,0.08)', boxShadow: '0 8px 32px rgba(11,26,51,0.04)' }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {MOBILIZATION.map(({ label, desc }) => (
            <div key={label} className="flex items-start gap-3">
              ...
              <div>
                <p className="font-bold text-[#0B1A33] text-sm mb-1">{label}</p>
                <p className="text-xs" style={{ color: 'rgba(11,26,51,0.65)' }}>{desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="w-full h-px mt-16" style={{ background: 'linear-gradient(90deg, transparent, rgba(201,150,44,0.3), transparent)' }} />
    </div>
  </section>
);
